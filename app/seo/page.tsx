      ...
      <div className="rounded-[2rem] border border-zinc-200/60 bg-white/70 p-6 dark:border-zinc-800 dark:bg-zinc-900/55">
        <header className="flex flex-col gap-4 border-b border-zinc-200/60 pb-8 dark:border-zinc-800 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-3xl border border-zinc-200/80 bg-white/80 px-4 py-3 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/80">
              <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-500">{t("seo.account", "Account")}</p>
              <p className="mt-2 text-sm font-medium">{activeAccountName}</p>
            </div>
            <div className="rounded-3xl border border-zinc-200/80 bg-white/80 px-4 py-3 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/80">
              <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-500">{t("seo.crawlDate", "Crawl date")}</p>
              <p className="mt-2 text-sm font-medium">12 May 2026</p>
            </div>
            <div className="rounded-3xl border border-zinc-200/80 bg-white/80 px-4 py-3 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/80">
              <p className="text-[10px] uppercase tracking-[0.16em] text-zinc-500">{t("seo.uploadStatus", "Upload status")}</p>
              <p className="mt-2 text-sm font-medium text-emerald-700 dark:text-emerald-300">{t("seo.latestCrawlProcessed", "Latest crawl processed")}</p>
            </div>
            <div className="rounded-3xl border border-zinc-200/80 bg-white/80 px-4 py-3 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/80">
              <p className="text-[10px] uppercase tracking-[0.18em] text-zinc-500">{t("seo.comparison", "Comparison")}</p>
              <p className="mt-2 text-sm font-medium">{t("seo.comparedWith", "Compared with 30 Apr 2026")}</p>
            </div>
          </div>
        </header>
        ...
        <section className="py-8">
          <div className="mb-4 rounded-3xl border border-zinc-200/80 bg-white/80 px-5 py-4 shadow-sm backdrop-blur dark:border-zinc-800 dark:bg-zinc-900/80">
            <p className="text-xs uppercase tracking-[0.22em] text-zinc-500">{t("seo.activeAccount", "Active account")}</p>
            <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-300">
              {t("seo.linkedTo", "Screaming Frog uploads are linked to")} 
              <span className="font-medium text-zinc-900 dark:text-zinc-100">{activeAccountName}</span>.
            </p>
          </div>
